...
revised 2015-03-19 by Qing Luo
"""
...
class UnivariateBayesianInference(object):
    ...
    def laplace(self, g=None):
        ...
        # YOUR CODE HERE CALCULAT & RETURN ampl, locn, sig, est
        seconddifference=(gqvals[i_max+1]-2*gqvals[i_max]+gqvals[i_max-1])/self.delta**2
        sig=sqrt(-gqvals[i_max]/seconddifference)
        est=gqvals[i_max]*sig*rt2pi 
    ...
    def samp_ar(self):
        ...
        method.
        """
        xrandom=np.random.uniform(self.param_grid[0],self.param_grid[-1])
        i_x = self.param_grid.searchsorted(xrandom) 
        yrandom=np.random.uniform(0,self.max_pdf)
        while yrandom > self.post_pdf[i_x] :
                xrandom=np.random.uniform(self.param_grid[0],self.param_grid[-1])
                i_x = self.param_grid.searchsorted(xrandom) 
                yrandom=np.random.uniform(0,self.max_pdf)
                self.n_ar= self.n_ar+1
        self.n_ar=self.n_ar+1
        return xrandom
